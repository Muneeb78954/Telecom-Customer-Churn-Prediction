...
# =============================================================================
# 9. SUBMISSION FILE EXPORT
# =============================================================================

def create_submission_file(test_raw: pd.DataFrame,
                            predictions: pd.DataFrame) -> None:
    """
    Create and export claude1submission.csv with ID and predicted churn.

    Takes the original test set and prediction results to create a
    properly formatted submission file for competition submission.
    """
    log.info("Creating submission file …")
    
    submission = pd.DataFrame({
        "id": test_raw.index,
        "churn": predictions["predicted_churn"].values,
    })
    
    submission_path = "claude1submission.csv"
    submission.to_csv(submission_path, index=False)
    log.info(f"  Submission saved → {submission_path}")
    log.info(f"  Total records: {len(submission):,}")
    return submission
...
def main():
    ...
    create_submission_file(test_raw, test_predictions)
